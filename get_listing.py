    ...
    async with httpx.AsyncClient() as client:
        for type_ in types:
            ...
            try:
                ...
                print(f"{datetime.now()}: Number of listing for {type_}: {len(df)}")
                dataframes.append(df)
            except Exception as e:
                print(f"{datetime.now()}: Error fetching data for type {type_}: {e}")

    # Combine all DataFrames and remove duplicates
    final_df = pd.concat(dataframes)
    final_df.drop_duplicates(inplace=True,subset = 'id')
    print(f"{datetime.now()}: Total number of listings with unique 'id' fetched: {len(final_df)}")
    return final_df
...
def load_to_db(df_listings):
    ...
    try:
        ...
        print(f'{datetime.now()}: Retrieved existing_ids')

        # Filter the DataFrame
        df_update = df_listings[df_listings['id'].isin(existing_ids)]
        df_insert = df_listings[~df_listings['id'].isin(existing_ids)]
        print(f'{datetime.now()}: Number of existing rows to be updated: {len(df_update)}')
        print(f'{datetime.now()}: Number of new rows to be updated: {len(df_insert)}')
        ...
        print(f'{datetime.now()}: Updated is_active to False for all current records')
        ...
        print(f'{datetime.now()}: Finished updating existing records')
        ...
        print(f'{datetime.now()}: Finished inserting new records')
...
def main():
    ...
    #Start timer
    start = perf_counter()
    print(f'{datetime.now()}: Start updating rental_listings table.')
    ...
    print(f'{datetime.now()}: Time spent in the main loop = {int(minutes)} minutes {int(seconds)} seconds')
