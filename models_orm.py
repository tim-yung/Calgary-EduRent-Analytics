...
class School(Base):
    ...
    email: Mapped[str] 
    website: Mapped[str] 
    school_hour: Mapped[str] 
    grades: Mapped[str] 
    ward: Mapped[int] 
    area: Mapped[int] 
    total_enrolment: Mapped[int] 
    programs_list: Mapped[str] 
    desc: Mapped[str] 
    kindergarten_enrolment: Mapped[int] 
    grade_1_enrolment: Mapped[int] 
    grade_2_enrolment: Mapped[int] 
    grade_3_enrolment: Mapped[int] 
    grade_4_enrolment: Mapped[int] 
    grade_5_enrolment: Mapped[int] 
    grade_6_enrolment: Mapped[int] 
    grade_7_enrolment: Mapped[int] 
    grade_8_enrolment: Mapped[int] 
    grade_9_enrolment: Mapped[int] 
    grade_10_enrolment: Mapped[int] 
    grade_11_enrolment: Mapped[int] 
    grade_12_enrolment: Mapped[int] 
    #schools_within_catchment_collection: Mapped[List['Schools_Within_Catchment']] = relationship(back_populates='school')
    attendance_areas_collection: Mapped[List['Attendance_Area']] = relationship(back_populates='school')
    walk_zones_collection: Mapped[List['Walk_Zones']] = relationship(back_populates='school')
...
class Attendance_Area(Base):
    ...
    attendance_area_id: Mapped[int] = mapped_column(primary_key=True)
    school_id: Mapped[int] = mapped_column(ForeignKey('school.school_id'))
    polygon_number: Mapped[int] 
    lat_coordinate: Mapped[float] 
    long_coordinate: Mapped[float] 
    school: Mapped[List['schools']] = relationship(back_populates='attendance_areas_collection')
    
    def __repr__(self) -> str:
        return f"Attendance_Area(school_id={self.school_id!r}, attendance_area_id={self.attendance_area_id!r},)"


class Walk_Zones(Base):
    __tablename__ = 'walk_zone'
    walk_zone_id: Mapped[int] = mapped_column(primary_key=True, nullable=True)
    school_id: Mapped[int] = mapped_column(ForeignKey('school.school_id'))
    polygon_number: Mapped[int] 
    lat_coordinate: Mapped[float] 
    long_coordinate: Mapped[float] 
    school: Mapped[List['schools']] = relationship(back_populates='walk_zones_collection')
    def __repr__(self) -> str:
        return f"Walk_Zones(school_id={self.school_id!r}, walk_zone_id={self.attendance_area_id!r},)"
...
'''
class Schools_Within_Catchment(Base):
    __tablename__ = 'schools_within_catchment'
    id: Mapped[int] = mapped_column(primary_key=True)
    listing_id: Mapped[int] = mapped_column(nullable=True, ForeignKey('rental_listings.id'))
    school_id: Mapped[int] = mapped_column(nullable=True, ForeignKey('schools.school_id'))
    school: Mapped[List['schools']] = relationship('schools', back_populates='schools_within_catchment_collection')
