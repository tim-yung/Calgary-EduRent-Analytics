...
def extract(conn):
    ...
    """
    try:
        print(f'{datetime.now()}: Obtaining new rental listings for matching...')
        ...
        df_new_listings = pd.read_sql_query("""
                                ...
                                """, conn)
        print(f'{datetime.now()}: New rental listings = {df_new_listings.shape[0]}')
        
        print(f'{datetime.now()}: Obtaining crime info from database...')
        df_crime = pd.read_sql_query("SELECT * FROM crime", conn)
    
    except Exception as e:
        print(f'{datetime.now()}: Error occurred during data extraction - {e}')
        raise
...
def transform(df_new_listings, df_crime):
    ...
    """
    print(f'{datetime.now()}: Transforming new listings with fuzzy matched community names...')
    ...
    print(f'{datetime.now()}: Finished transforming data in {end_perf - start_perf:.4f} seconds. Matched rows: {df_listing_with_crime.shape[0]}')
...
def load(conn, cursor, df_listing_with_crime):
    ...
    try:
        print(f'{datetime.now()}: Loading transformed data into the database...')
...
def main():
    ...
    """
    print(f'{datetime.now()}: Start checking crime data for rental listings.')
    ...
    try:
        ...
        print(f'{datetime.now()}: Time spent in loading crime data = {int(minutes)} minutes {int(seconds)} seconds')
    
    except Exception as e:
        print(f'{datetime.now()}: An error occurred in the ETL process - {e}')
